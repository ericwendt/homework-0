...
void fill_sin_wave(short waveform[NUM_SAMPLES]) {
        int i;
...
void add_noise(short waveform[NUM_SAMPLES]) {
        int i;
        short range, val;
...
void filter_sin_wave(short samples[NUM_SAMPLES], double weights[FILTER_WINDOW]) {
        struct ring *ring;
        short last;
        int sample;
        int ret;
...
int write_csv(const char *filename, short samples[NUM_SAMPLES]) {
        int i;
        int ret;
        size_t len;
        char line[32];
...
int write_samples(const char *filename, short samples[NUM_SAMPLES]) {
